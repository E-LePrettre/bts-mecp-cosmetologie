---
author: ELP
title: 06 🖊️ Évaluation
---

# 06 – Tensioactifs : rôle et usage en cosmétologie  
## 🖊️ Évaluation formative

**Analyser un produit lavant et argumenter le choix des tensioactifs**

⏱️ Durée : 30 minutes  
🎯 Objectif : évaluer la capacité à **analyser une composition cosmétique**, **mobiliser des notions sur les tensioactifs**, et **argumenter un choix de formulation**, conformément aux attendus de l’épreuve **E2 – Expertise scientifique et technologique**

📌 Évaluation formative – barème indicatif

---

## 📌 Statut de l’évaluation (E2)

Cette évaluation ne vise pas la restitution du cours.  
Elle évalue la capacité de l’étudiant à :

- analyser une **situation professionnelle**,
- identifier des **ingrédients fonctionnels**,
- mobiliser les notions de **tensioactifs et de tolérance cutanée**,
- **argumenter** un raisonnement cosmétologique.

Toute réponse non justifiée ou uniquement affirmative sera considérée comme **incomplète**.

---

## 🧴 Situation professionnelle

Vous travaillez dans un **laboratoire cosmétique**.

Un service marketing souhaite vérifier la **cohérence de la formulation** d’un gel douche destiné à un usage quotidien.

Votre mission est d’analyser la composition du produit et de **justifier le choix des tensioactifs**.

---

## Exercice 1 – Identification des tensioactifs  
### (4 points)

À partir du **Document 1**, répondez aux questions suivantes.

1. Relevez les **tensioactifs présents** dans la liste INCI.

<br>
<br>
<br>
<br>

2. Indiquez la **famille** de chacun des tensioactifs identifiés.

<br>
<br>
<br>
<br>

---

## Exercice 2 – Rôle et fonctionnement des tensioactifs  
### (4 points)

1. Définissez ce qu’est un **tensioactif**.

<br>
<br>
<br>
<br>

2. Expliquez brièvement comment les tensioactifs permettent le **nettoyage de la peau**.

<br>
<br>
<br>
<br>

---

## Exercice 3 – Choix des tensioactifs et tolérance cutanée  
### (6 points)

Le produit est destiné à un **usage quotidien**, sur **peaux sensibles**.

1. Le choix des tensioactifs vous paraît-il **adapté** à cet usage ?

☐ Oui  ☐ Non  

2. Justifiez votre réponse en vous appuyant sur :
- les **familles de tensioactifs**,
- la **tolérance cutanée** attendue,
- l’usage du produit.

Rédigez une réponse structurée (4 à 6 lignes).

<br>
<br>
<br>
<br>
<br>
<br>

---

## Exercice 4 – Analyse critique et argumentation professionnelle  
### (attendus BTS / E2) (4 points)

Proposez **une amélioration possible** de la formulation si le produit devait être destiné :
- soit à des **peaux très sensibles**,
- soit à un **usage fréquent chez l’enfant**.

Vous devez **argumenter votre proposition**.

<br>
<br>
<br>
<br>
<br>
<br>
<br>

---

## Exercice 5 – Posture professionnelle et E2  
### (2 points)

Un étudiant écrit :

> « Ce gel douche lave bien parce qu’il mousse beaucoup. »

1. Cette réponse est-elle suffisante pour un **niveau BTS MECP** et pour l’épreuve **E2** ?

☐ Oui  ☐ Non  

2. Citez **deux éléments scientifiques attendus** dans une réponse conforme aux attendus de l’épreuve **E2**.

<br>
<br>
<br>
<br>

---

## 📎 Documents supports (annexes)

## 📄 Document 1 – Liste INCI d’un gel douche

> Aqua, Sodium Laureth Sulfate, Cocamidopropyl Betaine, Glycerin, Sodium Chloride, Citric Acid, Parfum, Sodium Benzoate, Potassium Sorbate

---

## 🔍 Auto-évaluation

Cochez ce qui vous correspond :

☐ Je sais définir un tensioactif  
☐ Je sais identifier des tensioactifs dans une liste INCI  
☐ Je sais classer les tensioactifs par familles  
☐ Je sais relier un choix de tensioactifs à l’usage du produit  
☐ Je sais argumenter une réponse de niveau E2  
☐ Je dois encore progresser dans la justification écrite
